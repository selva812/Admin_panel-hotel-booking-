          ...
          <div ref={invoiceRef} className='invoice-content'>
            ...
            <div className='text-center mb-1'>
              <h2 className='text-md font-medium border-b pb-1'>
                {invoice.charges?.tax === '0.00' ? 'BILL' : 'GST INVOICE'}
              </h2>
            </div>
            ...
            <div className='grid grid-cols-2 gap-3 border p-3 mb-4 text-xs'>
              <div className='space-y-1'>
                ...
                {invoice.customer?.gst && (
                  <p className='truncate'>
                    <strong className='inline-block w-24'>GST No:</strong>
                    {invoice.customer.gst}
                  </p>
                )}
                <p>
              ...
              <div className='space-y-1'>
                ...
                <p>
                  ...
                  {invoice.bookingDetails?.rooms?.map(room => room.roomNumber).join(', ')}
                </p>
              </div>
            </div>
            ...
            <table className='w-full border mb-4 text-xs'>
              ...
              <tbody>
                {/* Room Charges with Room Numbers */}
                {invoice.bookingDetails?.rooms?.map((room, index) => (
                  <tr key={index}>
                    <td className='px-1 py-0.5 border'>
                      Room {room.roomNumber} - {room.type} ({room.nights} night{room.nights > 1 ? 's' : ''})
                    </td>
                    <td className='px-1 py-0.5 border text-right'>{room.price.toFixed(2)}</td>
                  </tr>
                ))}
                ...
                {/* GST with percentage - only for rooms and extra beds */}
                {invoice.charges?.tax !== '0.00' && (
                  <tr>
                    <td className='px-1 py-0.5 border'>
                      GST ({invoice.charges?.tax_percentage}%) - Applicable on Room & Extra Bed charges only
                    </td>
