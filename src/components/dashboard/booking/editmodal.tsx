...
interface EditCustomerModalProps {
    isOpen: boolean
    onClose: () => void
    onSave: (customer: { id: number; phone: string; name: string; company: string }) => void
...
export default function EditCustomerModal({ isOpen, onClose, onSave, customerId }: EditCustomerModalProps) {
    ...
    useEffect(() => {
        const fetchCustomer = async () => {
            ...
            try {
                const response = await fetch(`/api/customers/edit?id=${customerId}`);
                ...
                if (data.success) {
                    ...
                    setForm({
                        ...
                        idProofNumber: customer.idNumber || '',
                        address: customer.address || '',
                        gstno: customer.gst_no || ''
                    });

                    // Set existing images if available
                    if (customer.idPicture) {
                        setAdhaarPreview(customer.idPicture);
                    }
                    if (customer.picture) {
                        setProfilePreview(customer.picture);
                    }
                } else {
                    setMessage({ text: data.message || 'Failed to load customer data', type: 'error' });
                }
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        try {
            ...
            formData.append('gstno', form.gstno);
            ...
            setMessage({ text: data.message || 'Customer updated successfully', type: 'success' });
                ...
                <div className="p-6 sm:p-8 relative">
                    ...
                    {initialLoading ? (
                        <div className="flex items-center justify-center py-8">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#C59F56]"></div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="form-group">
                                        ...
                                        <label className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-[#C59F56] transition-colors">
                                            ...
                                            />
                                            {adhaarPreview ? (
                                                <img src={`/uploads/${adhaarPreview}`} alt="Aadhaar preview" className="h-full w-full object-cover rounded-lg" />
                                            ) : (
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-[#C59F56] transition-colors relative">
                                            ...
                                            >
                                                {profilePreview ? (
                                                    <img src={`/uploads/${profilePreview}`} alt="Profile preview" className="h-full w-full object-cover rounded-lg" />
                                                ) : (
                            ...
                            <div className="pt-6 text-center">
                                <button
                                    ...
                                    className={`bg-[#C59F56] text-white font-semibold px-8 py-3 rounded-lg transition-colors ${loading || initialLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#b88b3c]'
                                        }`}
                                >
                                    {loading ? (
